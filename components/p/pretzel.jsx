import { Icon } from '@iconify/css-react';
import { createElement } from 'react';

const viewBox = {"width":36,"height":36};

/** @param {{width?: string; height?: string;}} */
function Component({width, height, ...props}) {
	return createElement(Icon, {
		...props,
		width,
		height,
		viewBox,
		"content": `<style>.a_14ip2ko {
  cx: 7px;
  cy: 22px;
  r: 1px;
}

.b3etxccqh {
  cx: 6px;
  cy: 11px;
  r: 1px;
}

.cjifmub5z {
  cx: 3px;
  cy: 14px;
  r: 1px;
}

.ctwquuh2f {
  cx: 21px;
  cy: 26px;
  r: 1px;
}

.f0xae0btc {
  cx: 22px;
  cy: 8px;
  r: 1px;
}

.g2bow2b8t {
  cx: 31px;
  cy: 9px;
  r: 1px;
}

.hax8mkbsy {
  cx: 19px;
  cy: 9px;
  r: 1px;
}

.kh2cv7ptv {
  fill: var(--svg-color--ad5810, #ad5810);
  d: path("M11.751 22.764c-.794.448-1.78 1.172-2.376 2.236c-.688 1.229-1.218 2.033-1.584 2.544q.503.312 1.032.581a30 30 0 0 1 1.552-2.688c.768-1.179 1.902-1.912 2.489-2.238a12 12 0 0 1-1.113-.435m11.692-.434l-.005.002q.472.17.886.402c-.19.09-.399.153-.597.233c.564.415 1.348 1.092 1.96 2.032a61 61 0 0 1 1.797 2.952c.26-.14.521-.278.771-.435c.188.3.372.567.562.792c0 0-.562-.792-2.004-3.31s-3.37-2.668-3.37-2.668m-2.393-3.531c.05-.055.09-.112.138-.168a93 93 0 0 0-2.023 1.622a5.7 5.7 0 0 1-1.161.724c.401.18.839.335 1.296.477c.524-.293 1.157-.698 1.701-1.204c.56-.521.892-.92 1.081-1.23c-.029-.005-.054-.015-.082-.02a12 12 0 0 1-.95-.201m-2.467-6.088c.732-.612 2.127-1.524 3.951-1.669a8 8 0 0 0-.376-.935c-.583.091-1.385.249-2.158.518c-.877.305-1.58.914-1.998 1.344c.235.236.423.486.581.742");
}

.n-_j57bvp {
  cx: 25px;
  cy: 27px;
  r: 1px;
}

.sssgtebge {
  cx: 14px;
  cy: 27px;
  r: 1px;
}

.v33bpbn0h {
  fill: var(--svg-color--ffe8b6, #ffe8b6);
}

.vsw8u7b8o {
  cx: 12px;
  cy: 7px;
  r: 1px;
}

.yjy-_gbfd {
  fill: var(--svg-color--f4900c, #f4900c);
  d: path("M35 16c0-8-6-11-12-11c-1.738 0-3.472.371-5 1.097C16.472 5.371 14.738 5 13 5C7 5 1 8 1 16c0 3 1.314 5.662 3 8c.434.602.903 1.146 1.4 1.647c-.357.603-.616 1.091-.779 1.353C4 28 5 29 6 29c.69 0 1.205-.617 1.745-1.482C10.552 29.273 14.023 30 18 30s7.448-.727 10.255-2.482C28.795 28.383 29.31 29 30 29c1 0 2-1 1.379-2c-.163-.262-.422-.75-.779-1.353c.497-.501.967-1.046 1.4-1.647c1.686-2.338 3-5 3-8m-17 1.207C16.578 15.736 17 14 17 14c0-.708.381-1.414 1-2.034c.619.62 1 1.326 1 2.034c0 0 .422 1.736-1 3.207M8.215 19.94C6.59 17.687 6.204 15.388 7 13c1.053-3.158 4.85-3 6-3c.288 0 .576.024.861.059C13.314 11.185 13 12.498 13 14c0 1.476.578 3.273 1.951 4.799c-.298.075-.61.144-.951.201c-1.907.318-3.471 1.122-4.745 2.107a8 8 0 0 1-1.04-1.167M18 24c-2.296 0-4.476-.394-6.324-1.266A5.8 5.8 0 0 1 14 22c1.565-.157 2.885-.524 4-1.024c1.115.5 2.435.867 4 1.024a5.8 5.8 0 0 1 2.324.734C22.476 23.606 20.296 24 18 24m9.785-4.06a8.3 8.3 0 0 1-1.04 1.167C25.471 20.122 23.906 19.318 22 19a12 12 0 0 1-.95-.201C22.422 17.273 23 15.476 23 14c0-1.502-.313-2.815-.861-3.941c.285-.035.573-.059.861-.059c1.15 0 4.947-.158 6 3c.796 2.388.41 4.687-1.215 6.94");
}

.yl1qe6bgs {
  cx: 31px;
  cy: 17px;
  r: 1px;
}
</style><path class="yjy-_gbfd"/><path class="kh2cv7ptv"/><g class="v33bpbn0h"><circle class="a_14ip2ko"/><circle class="cjifmub5z"/><circle class="b3etxccqh"/><circle class="vsw8u7b8o"/><circle class="sssgtebge"/><circle class="ctwquuh2f"/><circle class="n-_j57bvp"/><circle class="yl1qe6bgs"/><circle class="g2bow2b8t"/><circle class="f0xae0btc"/><circle class="hax8mkbsy"/></g>`,
		"fallback": "twemoji:pretzel",
	});
}

export default Component;
