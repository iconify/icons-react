# @iconify-react/bpmn

Icon set prefix: **bpmn**

Icon set name: **BPMN**

Author: **Camunda Services GmbH**

License: **[Open Font License](https://github.com/bpmn-io/bpmn-font/blob/master/LICENSE)**

Repository: **https://github.com/bpmn-io/bpmn-font**

Browse all icons: **[Preview BPMN](https://icon-sets.iconify.design/bpmn/)**

## Installation

```bash
npm install github:iconify/icons-react#bpmn
```

## Usage

For full documentation visit [Iconify website](https://iconify.design/docs/usage/svg-css/react/).

Few usage examples:

```jsx
import SampleIcon from '@iconify-react/bpmn/intermediate-event-catch-non-interrupting-escalation';

function Test() {
  return (
    <SampleIcon />
  );
}
```

To resize icon, set `width` and/or `height` parameters:

```jsx
import SampleIcon from '@iconify-react/bpmn/user';

function Test() {
  return (
    <SampleIcon height="1em" />
  );
}
```

To change icon color, use style:

```jsx
import SampleIcon from '@iconify-react/bpmn/lane-insert-above';

function Test() {
  return (
    <SampleIcon style={{color: 'red'}} />
  );
}
```
