import { Icon } from '@iconify/css-react';
import { createElement } from 'react';
import '../../css/f/ft5dv1b6b.css';
import '../../css/f/f5mtcdbpm.css';
import '../../css/k/k59-kw8vi.css';
import '../../css/p/p_x7miasa.css';
import '../../css/h/h734a6bjw.css';
import '../../css/r/r-7p2ab6x.css';
import '../../css/r/rk7-b4w8q.css';

const viewBox = {"width":227,"height":228};

/** @param {{width?: string; height?: string;}} */
function Component({width, height, ...props}) {
	return createElement(Icon, {
		...props,
		width,
		height,
		viewBox,
		"content": `<g class="ft5dv1b6b"><path class="f5mtcdbpm"/><path class="k59-kw8vi"/><path class="p_x7miasa"/><path class="h734a6bjw"/><path class="r-7p2ab6x"/><path class="rk7-b4w8q"/></g>`,
		"fallback": "thesvg-color:home-depot",
	});
}

export default Component;
