import { Icon } from '@iconify/css-react';
import { createElement } from 'react';
import '../../css/d/dqdx2eb9s.css';
import '../../css/r/rl3jt7bei.css';
import '../../css/x/xpdx163qj.css';
import '../../css/s/s7s5cybow.css';
import '../../css/l/lpidr29by.css';
import '../../css/q/qugbkgbza.css';
import '../../css/d/dfbjuqlpe.css';
import '../../css/m/mhmit0iox.css';
import '../../css/a/a50-tubva.css';

const viewBox = {"width":128,"height":128};

/** @param {{width?: string; height?: string;}} */
function Component({width, height, ...props}) {
	return createElement(Icon, {
		...props,
		width,
		height,
		viewBox,
		"content": `<path class="dqdx2eb9s"/><path class="rl3jt7bei"/><path class="xpdx163qj"/><path class="s7s5cybow"/><path class="lpidr29by"/><path class="qugbkgbza"/><path class="dfbjuqlpe"/><path class="mhmit0iox"/><circle class="a50-tubva"/>`,
		"fallback": "devicon:apacheairflow",
	});
}

export default Component;
